...
The following are 5 subcommands (more could be added), 
each is a thin wrapper over a service: 
    - tradejournal init: create or migrate the db 
    - tradejournal sync: ingest from one or both venues
    - tradejournal rebuild: reconstruction (legs, pairing)
    - tradejournal report: summarization 
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="tradejournal",
        description=(
            "Read-only journal for delta-neutral perp positions."
        ),
    )
...
def _command_report(args: argparse.Namespace, settings: Settings) -> int:
    ...
    if not trades:
        print("No trades yet. Run `tradejournal sync` then `rebuild`.")
        connection.close()
        return 0
